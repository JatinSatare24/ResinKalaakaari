...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (
      record.status !== "verifying_payment" ||
      old_record?.status === "verifying_payment"
    ) {
    ...
    doc.text(
      "Pimple gurav pune 411061, Maharashtra | resinkalaakaari@gmail.com",
      14,
      30,
    );
    ...
    const splitAddress = doc.splitTextToSize(
      fullShippingAddress || "No address provided",
      85,
    );
    ...
    const qrCodeDataUri = await QRCode.toDataURL(upiString, {
      margin: 1,
      scale: 4,
    });
    const qrSize = 32;
    ...
    doc.text(
      "• Shipping: Resin Kalaakaari is not responsible for courier delays.",
      19,
      termsY + 18,
    );

    doc.text("Thank you for supporting Resin Kalaakaari!", 105, 288, {
      align: "center",
    });
    ...
    await resend.emails.send({
      ...
      attachments: [
        {
          filename: `Invoice_RK_${record.id.slice(0, 8)}.pdf`,
          content: pdfBase64,
        },
      ],
    });
